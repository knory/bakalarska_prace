...
namespace NongamifiedScenes
{
    public class Time : DoubleDropdownComponent
    {
        ...
        {
            var options1 = new DropdownModel[Constants.TIME_OPTIONS_COUNT];
            var options2 = new DropdownModel[Constants.TIME_OPTIONS_COUNT];

            for (int i = 0; i < Constants.TIME_OPTIONS_COUNT; i++)
            {
                options1[i] = new DropdownModel { Id = i, Text = (i * 2).ToString() };
                options2[i] = new DropdownModel { Id = i, Text = (i * 5).ToString("00") };
            }
